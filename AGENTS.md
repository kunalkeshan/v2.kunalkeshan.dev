...
- [ ] **If a new public-facing page/route was added, or an existing one's metadata/content changed**: it renders appropriate JSON-LD via `apps/web/components/shared/json-ld.tsx` + a `build*JsonLd` function in `apps/web/lib/structured-data.ts`, sets `alternates: { canonical }` in its metadata, and — if Sanity-backed with its own detail page — respects the document's `seo.noindex`/`seo.metaTitle` fields where that document type has an `seo` field. A new Sanity collection with detail pages is also added to `apps/web/app/sitemap.ts`'s per-collection chunks. This is a **strict, blocking rule**, same severity as the Sanity typegen rule above. See `docs/seo.md`.
- [ ] **If a new interactive/trackable element was added or an existing one's behavior changed** (a link, button, CTA, form submission, or other click/interaction in `apps/web`): consider whether it needs a custom analytics event via `apps/web/lib/analytics.ts` (`trackLinkClick`, `trackFormEvent`, `trackUiEvent`) — dual-fired to Google Analytics and Microsoft Clarity — rather than left untracked. This is a **strict, blocking rule**, same severity as the Sanity typegen rule above. See `docs/runbooks/analytics.md`.
