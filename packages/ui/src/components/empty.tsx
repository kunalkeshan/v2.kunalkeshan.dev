...
function Empty({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="empty"
      className={cn(
        "flex w-full min-w-0 flex-1 flex-col items-center justify-center gap-4 rounded-xl border-3 border-dashed border-border p-6 text-center text-balance",
        className
      )}
      {...props}
    />
...
function EmptyTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "font-heading text-sm font-black tracking-tight",
        className
      )}
      {...props}
    />
