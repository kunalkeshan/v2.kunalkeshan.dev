...
`apps/studio/structure.ts` customizes the Studio's document list to surface the two singletons (`siteConfig`, `faqs`) at the top, followed by `legal`, then the orderable lists (`skill`, `service`, `value`, `experience`, `organization`, `publication`), followed by anything else.
