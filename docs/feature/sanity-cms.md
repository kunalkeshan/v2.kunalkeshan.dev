...
- `project` — orderable document: shipped product work, freelance builds, personal/open-source/research/college projects, covered by one `kind`-discriminated type rather than separate schemas (see the file's own header comment in `apps/studio/schemaTypes/projectType.ts`). Repo links use two distinct fields, not one: `githubRepo` (`"owner/name"`, singular) is the **primary** repo — the only one whose star count is fetched and shown as a badge; `additionalRepos[]` (`{ repo, label? }`, each also `"owner/name"`) covers any other repos the project spans (e.g. a companion CLI), rendered as plain link chips with no star count. Both are distinct from the generic `links[]` array's `type: "repo"` option, which is a free-form URL + label for the rare case that doesn't fit the owner/name model.
